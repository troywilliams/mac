...
#
BACKUP_NAME="dfbkup-$(date +%Y%m%dT%H%M%S)"
...
main() {
    ...
    for i in ${FILES_TO_SYMLINK[@]}; do
        ...
        if [ -e "$targetFile" ]; then
            if [ "$(readlink "$targetFile")" != "$sourceFile" ]; then
                ...
                ask_for_confirmation "'$targetFile' already exists, do you want to backup and overwrite it?"
                if answer_is_yes; then
                    if ! [ -d "$BACKUP_DIRECTORY" ]; then
                      mkdir -p "$BACKUP_DIRECTORY"
                    fi
                    cp -Rf "$targetFile" "$BACKUP_DIRECTORY"
                    rm -rf "$targetFile"
    ...
    if  [ -d "$BACKUP_DIRECTORY" ]; then
      tar -czf "$HOME/$BACKUP_NAME.tgz" -C "$BACKUP_DIRECTORY" .
      if [ -f "$HOME/$BACKUP_NAME.tgz" ]; then
        rm -Rf "$BACKUP_DIRECTORY"
        print_success "Files archived to $HOME/$BACKUP_NAME.tgz"
      fi
    fi
}
